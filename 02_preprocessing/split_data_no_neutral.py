"""
Split a CSV dataset into train, validation, and test sets, excluding neutral ratings.

Rows where overall_rating == 3 are dropped before splitting.

Usage:
    python split_data_no_neutral.py --input <path> [options]

Examples:
    python split_data_no_neutral.py --input data/raw/B2W-Reviews01.csv
    python split_data_no_neutral.py --input data/raw/B2W-Reviews01.csv --output-dir data/processed --seed 42
"""
...
def split_dataset_no_neutral(
    ...
    before = len(df)
    df = df[df["overall_rating"] != NEUTRAL_RATING].reset_index(drop=True)
    print(f"  Dropped {before - len(df):,} rows with overall_rating == {NEUTRAL_RATING}")
    ...
    stem = input_path.stem + "_no_neutral"
    splits = {
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Split a CSV dataset into train / val / test sets, excluding overall_rating == 3."
    )
